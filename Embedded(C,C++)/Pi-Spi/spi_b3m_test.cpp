...
#define MOTOR_DATA_ADDR     0
#define MOTOR_DATA_SIZE     12
...
int main(int argc, char** argv)
{
  ...
  ret = spi_interface.read(MOTOR_DATA_ADDR, rx, MOTOR_DATA_SIZE);
  if(ret < 0)
    printf("SPI read error.\n");

  for(int i =0 ; i< NUM_MOTORS; i++)
  {
    motor_data = (sMotorData *) (rx + i*MOTOR_DATA_SIZE);
    printf("id:%d\t, status:%d\t, p:%05d, v%05d, c%05d\n", 
        motor_data->id, 
        motor_data->status,
        motor_data->position,
        motor_data->velocity,
  ...
  ret = spi_interface.write(MOTOR_DATA_ADDR, tx, 38);
