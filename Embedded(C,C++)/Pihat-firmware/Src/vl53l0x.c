 ...
 *      Author: Prakhar Goel
 *      Based on the Polulo Arduino library and Marcel Mg's project
 */
 ...
 void print_msg(char*msg, ...)
{
	...
	UART_Transmit(buff, strlen((char*) buff), 25);
}
 /**
   * @brief This function pulls XSHUT LOW for a particular sensor
   * @param input sensor id as integer -> 1,2,3,4,5,6 etc....
   * @retval None
   */
void VL53L0X_Xshut_Deactivate(int sensor_id)
{
	GPIO_write(sensor_id,RESET);
}
/**
   * @brief This function pulls x shut pin to input state (high impedance state)
   * @param input sensor id in integer -> 1,2,3,4,5,6 etc...
   * @retval None
   */
void VL53L0X_Xshut_Activate(int sensor_id)
{
	GPIO_input_state(sensor_id);
}
/**
   * @brief This function sets the address of the sensor
   * @param object of structure VL53L0X (pointer)
   * @param new address
   * @retval None
   */
...
/**
   * @brief This function returns the address of the sensor
   * @param object of structure VL53L0X (pointer)
   * @retval address of sensor
   */
uint8_t VL530X_getAddress(struct VL53L0X * dev)
{
	return dev->address;
}
...
/**
   ...
   * @note This function does not perform reference SPAD calibration (VL53L0X_PerformRefSpadManagement()), since the API user manual says that it is performed by ST on the bare modules; it seems like that should work well enough unless a cover glass is added.
...
bool VL53L0X_init(struct VL53L0X* dev)
{
 ...
 VL53L0X_setMeasurementTimingBudget(dev, dev->measurement_timing_budget_us);
  ...
  if (!VL53L0X_performSingleRefCalibration(dev, 0x40))
  { return false; }
  ...
  if (!VL53L0X_performSingleRefCalibration(dev, 0x00))
  { return false; }
...
void VL53L0X_writeReg(struct VL53L0X* dev, uint8_t reg, uint8_t value)
{
	...
	buf[0] = reg;
	buf[1] = value;
	dev->last_status = I2C_memory_write(dev->address, buf[0], I2C_MEMADD_SIZE_8BIT, &value , 1, timeout_i2c);
}

/**
   * @brief writes an 16-bit register
   * @param object of structure VL53L0X (pointer)
   * @param 8 bit register address
   * @param 16 bit value
   * @retval None
...
void VL53L0X_writeReg16Bit(struct VL53L0X* dev, uint8_t reg, uint16_t value)
{
		...
		dev->last_status = I2C_memory_write( dev->address, reg, I2C_MEMADD_SIZE_8BIT, buf , 2, timeout_i2c);

}
/**
   * @brief writes an 32-bit register
   * @param object of structure VL53L0X (pointer)
   * @param 8 bit register address
   * @param 32 bit value
   * @retval None
...
void VL53L0X_writeReg32Bit(struct VL53L0X* dev, uint8_t reg, uint32_t value)
{
	   uint8_t buf[4];
		...
		dev->last_status = I2C_memory_write( dev->address, reg, I2C_MEMADD_SIZE_8BIT, buf , 4, timeout_i2c);
}
/**
...
{
  uint8_t value;
  dev ->last_status = I2C_memory_read(dev->address, reg, I2C_MEMADD_SIZE_8BIT, &value,1,timeout_i2c);
  return value;
}
/**
...
uint16_t VL53L0X_readReg16Bit(struct VL53L0X* dev, uint8_t reg)
{
  uint16_t value;
  uint8_t buf[2];
  dev ->last_status = I2C_memory_read(dev->address, reg, I2C_MEMADD_SIZE_8BIT, buf ,2,timeout_i2c);
...
uint32_t VL53L0X_readReg32Bit(struct VL53L0X* dev, uint8_t reg)
{
  uint32_t value;
  uint8_t buf[4];
  dev ->last_status = I2C_memory_read(dev->address, reg, I2C_MEMADD_SIZE_8BIT, buf,4,timeout_i2c );
...
/**
	 * @brief Write an arbitrary number of bytes from the given array to the sensor, starting at the given register
	 * @param object of structure VL53L0X (pointer)
	 * @param 8 bit register address
...
{
	dev->last_status = I2C_memory_write( dev->address, reg, reg>0xFF ? I2C_MEMADD_SIZE_16BIT : I2C_MEMADD_SIZE_8BIT, src, count, timeout_i2c);
}
/**
	 * @brief Read an arbitrary number of bytes from the sensor, starting at the given register, into the given array
	 * @param object of structure VL53L0X (pointer)
	 * @param 8 bit register address
...
{
	{
	     ...
	     I2C_transmit(dev->address, &reg, 1, timeout_i2c);
	     /* Receive multiple byte */
		 dev-> last_status = I2C_receive(dev->address, dst, count, timeout_i2c);
	 }
}

/** @brief Set the return signal rate limit check value in units of MCPS (Mega counts per second).
 *  @note "This represents the amplitude of the signal reflected from the target and detected by the device"; setting this limit presumably determines
 *  the minimum measurement necessary for the sensor to report a valid reading.
 *  Setting a lower limit increases the potential range of the sensor but also seems to increase the likelihood of getting an inaccurate reading because of
 *  unwanted reflections from objects other than the intended target.
 *  Defaults to 0.25 MCPS as initialized by the ST API and this library.
 *  @param pointer to 	VL53L0X structure
 *  @param floating point value for MCPS
 *  @retval None
 */
...
/**
 * @brief Get the signal rate limit check value in MCPS
 * @param pointer to structure VL53L0X
 * @retval floating point valaue of MCPS_Limit
 */
 ...
 *  @note the ST API and this library take care of splitting the timing budget among the sub-steps in the ranging sequence.
 *  A longer timing budget allows for more accurate measurements. Increasing the budget by a factor of N decreases the range measurement standard deviation by a factor of
 *  sqrt(N). Defaults to about 33 milliseconds; the minimum is 20 ms.
 *  based on VL53L0X_set_measurement_timing_budget_micro_seconds()
 *  @param pointer to VL53L0X structure
 *  @param Budget (32 bit unsigned integer)
 *  @retval Boolean
 */
 ...
 * @param pointer to structure VL53L0X
 * @retval 32 bit unsigned integer (budget)
 */
...
/**
 * @brief Set the VCSEL (vertical cavity surface emitting laser) pulse period for the given period type (pre-range or final range) to the given value in PCLKs.
 * @note  Longer periods seem to increase the potential range of the sensor. Valid values are (even numbers only):
 *        Pre:  12 to 18 (initialized default: 14)
 *        Final: 8 to 14 (initialized default: 10)
 *        Based on VL53L0X_set_vcsel_pulse_period()
 * @param pointer to structure VL53L0X
 * @param VL53L0X_vcselPeriodType : Pre range and final range
 * @param 8 bit unsigned integer: period
 * @retval Boolean
 */
...
/**
 * @brief Get the VCSEL PULSE PERIOD in PCLK'S for the given period type
 * @note Based on the VL53L0X_get_vcsel_pulse_period() from the API
 * @param Pointer to the structure VL53L0X
 * @param VL53L0X_vcselPeriodType
 ...
 * @note If period_ms (optional) is 0 or not given, continuous back-to-back mode is used (the sensor takes measurements as often as possible)
 ...
 * @param Pointer to the structure VL53L0X
 * @param unsigned integer 32 bits (period in milliseconds);
 * @retval None
 */
...
/**
 * @brief Stop continuous ranging measurement.
 * @note Based on VL53L0X_StopMeasurement()
 * @param Pointer to the structure VL53L0X
 * @retval None
 */
 ...
 * @note readRangeSingleMillimeters() also calls this function after starting a single-shot range measurement
 * @param Pointer to the structure VL53L0X
 * @retval unsigned integer 16 bits (distance in mm)
 */
...
/**
 * @brief Performs a single-shot range measurement and returns the reading in millimeters
 * @note  Based on VL53L0X_PerformSingleRangingMeasurement()
 * @param Pointer to the structure VL53L0X
 * @retval unsigned integer 16 bits (distance in mm)
 */
...
/**
 * @brief Did a timeout occur in one of the read functions since the last call to timeoutOccurred()?
 * @param pointer to structure VL53L0X
 * @retval boolean value
 */
...
/**
 * @brief Get reference SPAD (single photon avalanche diode) count and type based on VL53L0X_get_info_from_device(), but only gets reference SPAD count and type
 * @param pointer to structure VL53L0X
 * @param integer pointer to count
 * @param boolean pointer (type_is_aperture)
 * @retval boolean value
 */
 ...
 * @param pointer to structure VL53L0X
 * @param pointer to structure VL53L0X_SequenceStepEnables
 * @retval None
 */
 ...
 * @note Based on get_sequence_step_timeout(), but gets all timeouts instead of just the requested one, and also stores intermediate values
 * @param pointer to structure VL53L0X
 * @param pointer to structure VL53L0X_SequenceStepEnables
 * @param pointer to structure VL53L0X_SequenceStepTimeouts
 * @retval None
 */
 ...
 *         The original function returned a uint32_t, but the return value is always stored in a uint16_t.
 * @param unsigned integer 16 bits (register value)
 * @retval unsigned integer 16 bits
 */
 ...
 * @note   Based on VL53L0X_encode_timeout(), the original function took a uint16_t, but the argument passed to it is always a uint16_t..
 * @param  unsigned integer 16 bits ( timeout_mclks)
 * @retval unsigned integer 16 bits
 */
 ...
 * @param timeout_period in mclks (8 bit integer)
 * @param vcsel_period_pclks (16 bit integer)
 * @retval unsigned integer 32 bits
 */
 ...
 * @param Timeout_period in microseconds (32 bit integer)
 * @param vcsel_period_pclks (8 bit integer)
 * @retval unsigned integer 32 bits
 */
...
/**
 * @brief Based on VL53L0X_perform_single_ref_calibration()
 */
bool VL53L0X_performSingleRefCalibration(struct VL53L0X* dev, uint8_t vhv_init_byte)
{
 ...
 // VL53L0X_startTimeout(dev);
  uint32_t last_tick=get_tick_micro();
  while ((VL53L0X_readReg(dev,  RESULT_INTERRUPT_STATUS) & 0x07) == 0)
  {
    //if (VL53L0X_checkTimeoutExpired(dev))
	 if (get_tick_micro()-last_tick>50000)
    { return false; }
  }
...
{
	 dev->timeout_start_ms = get_tick_micro();
}
...
{
	 return (dev->io_timeout > 0 && (get_tick_micro() - dev->timeout_start_ms) > dev->io_timeout);
}
